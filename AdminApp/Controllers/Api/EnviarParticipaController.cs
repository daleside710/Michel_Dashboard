...
namespace AdminApp.Controllers.Api
{
        ...
        public async Task<IActionResult> EnviarAsync([FromBody] ParticipationEnviarVirtualViewModel model)
        {
            ...
            try
            {
                ...
                if (pais_par == "ES")
                {
                    url = idsession + "&z=" + id_par;
                    pieza = "96115";
                    if (tipo == "VIRTUAL") pieza = "96115";
                    asunto = "Equipate con las mejores prestaciones";
                }
                else
                {
                    url = idsession + "&z=" + id_par;
    ...
    [Produces("application/json")]
    [Route("api/EnviarResendParticipa_info")]
    public class EnviarResendParticipa_infoController : Controller
    {
        private IHostingEnvironment _hostingEnvironment;
        private readonly ApplicationDbContext _context;
        public IHttpContextAccessor _accessor;
        ...
        public EnviarResendParticipa_infoController(IHostingEnvironment hostingEnvironment,
            ApplicationDbContext context, IHttpContextAccessor accessor)
        {
            _hostingEnvironment = hostingEnvironment;
            _context = context;
        ...
        public IActionResult Enviar([FromBody] ParticipationEnviarResendViewModel_info model)
        {
            ...
            try
            {
                ...
                try
                {
                    var dni_par = model.dni_par;
                    participation.dni_par = dni_par;
                }
                catch { }
